    ...
    public void extractRenderState(GuiGraphicsExtractor gui, int mouseX, int mouseY, float delta) {
        ...
        if (entries.isEmpty()) {
            ...
            for (int i = 0; i < rows; i++) {
                ...
                gui.text(font, displayNameOf(e), nameX, textY, MasterScreen.TEXT, false);
                gui.text(font, clip(font, descOf(e), idX - descX - 16),
                        descX, textY, MasterScreen.TEXT_DIM, false);
                gui.text(font, Component.translatable("voxkbd.ui.config.kb.id", e.id),
