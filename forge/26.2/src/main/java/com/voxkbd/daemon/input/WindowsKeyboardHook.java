...
public final class WindowsKeyboardHook implements PhysicalInputCapture {
    ...
    private boolean shouldCapture(PhysicalKey pk, boolean down) {
        ...
        // Mod-owned switch / config bindings are ALWAYS pass-through (§3.3 "防止卡死"): the keycode
        // set holds GLFW codes, so compare it against the key's GLFW code, never the Windows VK.
